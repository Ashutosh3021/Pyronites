...
class TableQuery:
    ...
    def __init__(self, transport: HttpTransport, name: str) -> None:
        ...
        self._op: Optional[str] = None  # "select" | "update" | "delete"

    def eq(self, column: str, value: Any) -> "TableQuery":
        """Add a single equality filter (current backend supports one)."""
    ...
    def select(self) -> "TableQuery":
        """Prepare a list/select operation."""
        self._op = "select"
        return self
    ...
    def insert(self, row: Union[Dict[str, Any], List[Dict[str, Any]]]) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        Maps to ``POST /tables/{name}`` (one request per row when given a list).
        """
    ...
    def update(self, data: Dict[str, Any]) -> "TableQuery":
        """Prepare an update. Must be followed by ``.eq(\"id\", value)``."""
        if not data:
    ...
    def single(self) -> Dict[str, Any]:
        """Execute a select and return exactly one row (or raise)."""
    ...
    def get(self, row_id: str) -> Dict[str, Any]:
        """Fetch a single row by id. Convenience for ``GET /tables/{name}/{id}``."""
    ...
    def __iter__(self):
        """Allow ``for row in client.table(\"n\").select():``."""
    ...
    def _execute_update(self) -> Dict[str, Any]:
        ...
        return self._http.request(  # type: ignore[return-value]
            "PATCH",
            f"/tables/{self._name}/{self._filter_value}",
